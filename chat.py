...
from src.core import VectorStore, RAGIndexer, RAGSystem, LocalAIClient
from src.utils import config
from src.cli import CLIInterface, CommandHandler, IndexingHandler, ConnectionHandler


def main():
    """Main application entry point."""
    parser = argparse.ArgumentParser(description="Simple RAG Chat CLI with Multi-Provider AI Support")
    parser.add_argument('--reindex', action='store_true',
                       help='Force reindex all documents')
    parser.add_argument('--no-chat', action='store_true',
                       help='Only index documents, don\'t start chat')
    parser.add_argument('--provider', choices=['lmstudio', 'ollama', 'openai_compatible'],
    ...
    if not args.no_chat:
        if not connection_handler.test_ai_connection(ai_client):
            cli.console.print("[yellow]⚠️  Continuing without AI provider - indexing only[/yellow]")
            args.no_chat = True
    ...
    if args.no_chat or not indexing_success:
        cli.console.print("[green]✅ Indexing complete. Run without --no-chat to start chat.[/green]")
        return

    # Show index statistics
    index_stats = vector_store.get_stats()
    cli.console.print("\n[blue]📊 Index statistics:[/blue]")
    cli.console.print(f"[blue]   • Total documents: {index_stats['total_documents']}[/blue]")
